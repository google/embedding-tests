...
flags.DEFINE_integer('train_size', 10, 'Number of training data per attribute')
flags.DEFINE_integer('test_size', 1000, 'Number of test data per attribute')
...
flags.DEFINE_integer('top_attr', 100, 'Predict top k attr')
...
flags.DEFINE_float('wd', 0., 'Weight decay for attribution model')
...
def encode_sentences(train_sents, test_sents, unlabeled_sents):
  ...
  if FLAGS.model_name == 'skipthought':
    from skip_thoughts import configuration, encoder_manager
    model_dir = os.path.join(FLAGS.model_dir, 'skip')
    vocab_file = os.path.join(model_dir, 'vocab.txt')
    embedding_file = os.path.join(model_dir, 'embeddings.npy')
  ...
  elif FLAGS.model_name == 'quickthought':
    model = QuickThoughtModel(50001, 620, 1200, 1,  init_word_emb=None,
                              cell_type='LSTM', train=False)
    model_name = ThoughtModelNameFunc['quickthought'](FLAGS.encoder_epoch,
                                                      gamma=FLAGS.gamma)
    ...
    encoder_fn = lambda s: encoder.encode(s, bsize=query_size, tokenize=False)
    sess = None
  else:
  ...
  unlabeled_embs = []
  if FLAGS.model_name == 'quickthought':
...
def train_embedding_classifier(data, unlabeled_data, num_attr):
  ...
  def augment_unlabeled(u):
    ...
    u = batch_interpolation(u, alpha=0.75, random=True)
    return u
...
def main(_):
  split_word = FLAGS.model_name in {'textcnn', 'quickthought'}
  train_sents, train_authors, test_sents, test_authors,\
      unlabeled_sents, unlabeled_authors = bookcorpus_author_data(
        ...
        split_word=split_word, top_attr=FLAGS.top_attr)
